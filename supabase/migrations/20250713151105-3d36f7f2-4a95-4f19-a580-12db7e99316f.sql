...
-- PHASE 4: Create immediate fix for current partial state
...
BEGIN
  ...
  IF partner1_id IS NOT NULL AND org_abc_id IS NOT NULL THEN
    ...
    ON CONFLICT (user_id, organization_id) DO NOTHING;
    GET DIAGNOSTICS user_orgs_fixed = ROW_COUNT;
  END IF;
  ...
  IF sub1_id IS NOT NULL AND org_pipes_id IS NOT NULL THEN
    ...
    ON CONFLICT (user_id, organization_id) DO NOTHING;
    GET DIAGNOSTICS user_orgs_fixed = user_orgs_fixed + ROW_COUNT;
  END IF;

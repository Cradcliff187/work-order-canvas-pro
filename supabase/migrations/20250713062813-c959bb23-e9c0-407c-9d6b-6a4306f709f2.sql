-- Update seed_test_data function with comprehensive business data and clean structure
...
BEGIN
  ...
  INSERT INTO profiles (id, user_id, email, first_name, last_name, user_type, company_name, is_active, is_employee, hourly_cost_rate, hourly_billable_rate) VALUES
    (admin_user_id, v_user_id, 'admin@workorderpro.com', 'System', 'Administrator', 'admin', 'WorkOrderPro', true, true, 75.00, 125.00),
    (partner1_id, gen_random_uuid(), 'partner1@abc.com', 'Sarah', 'Johnson', 'partner', 'ABC Property Management', true, false, NULL, NULL),
    (partner2_id, gen_random_uuid(), 'partner2@xyz.com', 'Mike', 'Chen', 'partner', 'XYZ Commercial Properties', true, false, NULL, NULL),
    (sub1_id, gen_random_uuid(), 'plumber@pipesmore.com', 'Tom', 'Wilson', 'subcontractor', 'Pipes & More Plumbing', true, false, 45.00, 85.00),
    (sub2_id, gen_random_uuid(), 'electrician@sparks.com', 'Lisa', 'Anderson', 'subcontractor', 'Sparks Electric', true, false, 50.00, 95.00),
    (emp1_id, gen_random_uuid(), 'maintenance@workorderpro.com', 'Alex', 'Thompson', 'employee', 'WorkOrderPro', true, true, 35.00, 65.00)
  ON CONFLICT (user_id) 
  DO UPDATE SET 
  ...
  INSERT INTO work_orders (id, work_order_number, title, description, organization_id, trade_id, status, created_by, assigned_to, date_submitted, store_location, street_address, city, state, zip_code) VALUES
    ...
    (wo3_id, 'XYZ-101-001', 'HVAC System Maintenance', 'Quarterly maintenance check on main HVAC system', org_xyz_id, trade_hvac_id, 'received', admin_user_id, NULL, now() - interval '2 days', 'XYZ Corporate Tower', '321 Business Blvd', 'Corporate', 'NY', '10001'),
    (wo4_id, 'PFG-201-001', 'General Maintenance', 'Monthly facility maintenance tasks', org_premium_id, trade_general_id, 'completed', admin_user_id, emp1_id, now() - interval '10 days', 'Premium Luxury Mall', '111 Luxury Lane', 'Uptown', 'FL', '33101');
  ...
  INSERT INTO work_order_assignments (id, work_order_id, assigned_to, assigned_by, assignment_type, assigned_organization_id, notes) VALUES
    (assign1_id, wo1_id, sub1_id, admin_user_id, 'lead', org_pipes_id, 'Urgent repair needed'),
    (assign2_id, wo2_id, sub2_id, admin_user_id, 'lead', org_sparks_id, 'Standard electrical work');
  ...
  INSERT INTO work_order_reports (id, work_order_id, subcontractor_user_id, work_performed, materials_used, hours_worked, invoice_amount, invoice_number, status, notes) VALUES
    (report1_id, wo2_id, sub2_id, 'Installed 3 new electrical outlets in conference room', 'Outlets (3), wire, junction boxes', 4.5, 425.00, 'SPE-2024-001', 'approved', 'Work completed successfully'),
    (report2_id, wo4_id, emp1_id, 'Completed monthly maintenance checklist', 'Cleaning supplies, light bulbs', 6.0, 0.00, NULL, 'approved', 'Internal employee work');
  ...
  INSERT INTO work_order_attachments (work_order_id, work_order_report_id, file_name, file_url, file_type, uploaded_by_user_id) VALUES
    (wo1_id, NULL, 'before_photo.jpg', 'https://example.com/before.jpg', 'photo', admin_user_id),
    (NULL, report1_id, 'completion_photo.jpg', 'https://example.com/complete.jpg', 'photo', sub2_id),
    (NULL, report2_id, 'maintenance_checklist.pdf', 'https://example.com/checklist.pdf', 'document', emp1_id);
  ...
  INSERT INTO invoices (id, internal_invoice_number, external_invoice_number, subcontractor_organization_id, status, submitted_by, submitted_at, total_amount) VALUES
    (inv1_id, 'INV-2024-00001', 'SPE-2024-001', org_sparks_id, 'approved', sub2_id, now() - interval '1 day', 425.00),
    (inv2_id, 'INV-2024-00002', 'PMP-2024-001', org_pipes_id, 'submitted', sub1_id, now() - interval '2 hours', 350.00);
  ...
  INSERT INTO invoice_attachments (invoice_id, file_name, file_url, file_type, uploaded_by) VALUES
    (inv1_id, 'invoice_SPE-2024-001.pdf', 'https://example.com/invoice1.pdf', 'invoice', sub2_id),
    (inv2_id, 'invoice_PMP-2024-001.pdf', 'https://example.com/invoice2.pdf', 'invoice', sub1_id);
  ...
  INSERT INTO employee_reports (employee_user_id, work_order_id, report_date, hours_worked, hourly_rate_snapshot, work_performed, notes) VALUES
    (emp1_id, wo4_id, (now() - interval '10 days')::date, 6.0, 35.00, 'Monthly facility maintenance', 'Completed all scheduled tasks'),
    (emp1_id, wo1_id, (now() - interval '2 days')::date, 2.0, 35.00, 'Initial assessment of plumbing issue', 'Confirmed leak, scheduled contractor');

  -- 13. Insert Receipts
  INSERT INTO receipts (id, employee_user_id, vendor_name, amount, receipt_date, description, notes) VALUES
    (receipt1_id, emp1_id, 'Home Depot', 45.67, (now() - interval '3 days')::date, 'Cleaning supplies and light bulbs', 'For monthly maintenance'),
    (receipt2_id, emp1_id, 'Ace Hardware', 23.45, (now() - interval '1 day')::date, 'Emergency plumbing supplies', 'Temporary fix materials');

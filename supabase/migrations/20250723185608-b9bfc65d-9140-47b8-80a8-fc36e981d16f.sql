...
-- Step 3: Create function to sync metadata when organizations change
...
-- Step 4: Recreate profile trigger to fire on both INSERT and UPDATE
CREATE TRIGGER sync_user_type_to_auth
  AFTER INSERT OR UPDATE ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_auth_user_metadata();

-- Step 5: Create trigger for user_organizations changes
...
-- Step 6: Create function to initialize missing JWT metadata for existing users
...
-- Step 7: Create function callable from client to trigger metadata sync
...
-- Step 8: Initialize metadata for all existing users

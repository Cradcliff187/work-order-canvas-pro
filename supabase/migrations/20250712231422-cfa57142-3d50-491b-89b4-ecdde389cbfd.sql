-- Fix clear_test_data function to include missing invoice-related tables
-- and correct the deletion cascade order
...
BEGIN
  ...
  -- Step 6: Delete invoice_work_orders (CRITICAL: Must be before work_orders deletion)
  WITH deleted AS (
    DELETE FROM public.invoice_work_orders 
    WHERE work_order_id = ANY(test_work_order_ids)
       OR invoice_id IN (
         SELECT id FROM public.invoices 
         WHERE subcontractor_organization_id = ANY(test_org_ids)
            OR submitted_by = ANY(test_user_ids)
       )
    RETURNING *
  )
  ...
  -- Step 7: Delete invoice_attachments (before invoices)
  WITH deleted AS (
    DELETE FROM public.invoice_attachments 
    WHERE uploaded_by = ANY(test_user_ids)
       OR invoice_id IN (
         SELECT id FROM public.invoices 
         WHERE subcontractor_organization_id = ANY(test_org_ids)
            OR submitted_by = ANY(test_user_ids)
  ...
  -- Step 8: Delete receipts and receipt work orders
  WITH deleted AS (
  ...
  -- Step 9: Delete invoices (after invoice_work_orders and invoice_attachments)
  WITH deleted AS (
  ...
  -- Step 10: Delete work orders (now safe after invoice tables are cleared)
  WITH deleted AS (
  ...
  -- Step 11: Delete partner locations
  WITH deleted AS (
  ...
  -- Step 12: Delete user organization relationships
  WITH deleted AS (
  ...
  -- Step 14: Delete profiles (this will cascade to auth.users)
  WITH deleted AS (

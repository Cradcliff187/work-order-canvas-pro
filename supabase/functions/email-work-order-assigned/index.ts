...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Processing work order assignment notification for:', work_order_id)
    ...
    if (!workOrder.assigned_profile) {
      throw new Error('No assigned user found for work order')
    }
    ...
    const variables = {
      subcontractorName: `${workOrder.assigned_profile.first_name} ${workOrder.assigned_profile.last_name}`,
    ...
    // Send email to assigned subcontractor
    try {
      const emailResponse = await resend.emails.send({
        from: 'WorkOrderPro <notifications@workorderpro.com>',
        to: [workOrder.assigned_profile.email],
        subject,
        html,
      })
      ...
      if (emailResponse.error) {
        await logEmail(supabase, {
          ...
          recipient_email: workOrder.assigned_profile.email,
          status: 'failed',
      ...
      console.log('Assignment email sent successfully to:', workOrder.assigned_profile.email)

      return new Response(
        JSON.stringify({ 
          success: true, 
          message_id: emailResponse.data?.id,
          recipient: workOrder.assigned_profile.email,
          work_order_id 
        }),
        { 
          status: 200, 

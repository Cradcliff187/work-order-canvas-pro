-- Phase 3: Advanced User Experience - Full-text Search & Filter Presets (Fixed)
...
-- Admins can view global presets (fixed casting)
CREATE POLICY "Admins can view global filter presets"
  ON filter_presets FOR SELECT
  USING (is_global = true AND has_internal_role(ARRAY['admin'::organization_role, 'manager'::organization_role]));

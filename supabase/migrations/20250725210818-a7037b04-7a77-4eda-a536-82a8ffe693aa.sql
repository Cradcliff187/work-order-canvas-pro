-- Enhanced Work Order Status Transition System Migration
...
ALTER TYPE work_order_status ADD VALUE IF NOT EXISTS 'estimate_approved' AFTER 'estimate_needed';
...
-- 3. Backup current function for rollback safety
CREATE OR REPLACE FUNCTION public.transition_work_order_status_backup()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
BEGIN
  -- This is a backup of the original function before enhancement
  -- Call the send email trigger for status changes
  PERFORM public.call_send_email_trigger(
    CASE 
      WHEN NEW.status = 'assigned' THEN 'work_order_assigned'
      WHEN NEW.status = 'completed' THEN 'work_order_completed'
      ELSE NULL
    END,
    NEW.id,
    'work_order'
...
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'Work order status transition trigger failed for %: %', NEW.id, SQLERRM;
  RETURN NEW;
END;
$function$;

-- Fix Storage RLS Policies for Work Order Attachments (Revised)
...
-- Helper function to extract work_order_id from file path (public schema)
CREATE OR REPLACE FUNCTION public.get_work_order_id_from_path(file_path text)
RETURNS uuid
LANGUAGE plpgsql
...
-- Helper function to extract user_id from legacy file paths (public schema)
CREATE OR REPLACE FUNCTION public.get_user_id_from_legacy_path(file_path text)
RETURNS uuid
LANGUAGE plpgsql
STABLE SECURITY DEFINER
AS $$
...
CREATE POLICY "partner_work_order_access"
  ...
  USING (
    ...
    AND (
      ...
      (public.get_work_order_id_from_path(name) IS NOT NULL
       AND EXISTS (
         SELECT 1 
         FROM work_orders wo
         JOIN organization_members om ON om.organization_id = wo.organization_id
         JOIN organizations o ON o.id = om.organization_id
         WHERE wo.id = public.get_work_order_id_from_path(name)
         AND om.user_id = auth_profile_id_safe()
         AND o.organization_type = 'partner'
       ))
      OR
      -- Legacy path structure: own uploads only
      (public.get_user_id_from_legacy_path(name) IS NOT NULL
       AND public.get_user_id_from_legacy_path(name) IN (

-- Email System Health Check Migration (Corrected)
...
-- List all database triggers with 'email' in the name (corrected query)
SELECT 
    ...
    CASE 
        WHEN t.tgname LIKE '%email%' THEN 'EMAIL_RELATED'
        WHEN t.tgname LIKE '%message%' THEN 'MESSAGE_RELATED'
        ELSE 'OTHER'
    END as trigger_category
FROM pg_trigger t
JOIN pg_class c ON t.tgrelid = c.oid
JOIN pg_namespace n ON c.relnamespace = n.oid
WHERE t.tgname ILIKE '%email%' 
   OR t.tgname ILIKE '%message%'

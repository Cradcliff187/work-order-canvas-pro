    ...
    const generateWorkOrderNumber = async () => {
      ...
      try {
        const { data: organization, error: orgError } = await supabase
          .from('organizations')
          .select('name, initials, uses_partner_location_numbers, next_work_order_sequence')
        ...
        if (!organization.uses_partner_location_numbers) {
          ...
          const { data: nextLocationCode, error: nextLocationCodeError } = await supabase.rpc(
            'increment_work_order_sequence',
            { organization_id: organizationId }
          );

          if (nextLocationCodeError) {
            throw new Error(`Error incrementing sequence: ${nextLocationCodeError.message}`);
          }
          ...
          autoGeneratedLocationCode = String(nextLocationCode).padStart(3, '0');
        ...
        const sequence = organization.next_work_order_sequence || 1;

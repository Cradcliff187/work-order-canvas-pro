...
// Admin-specific form data interface
interface AdminReceiptFormData {
...
const canSubmitForm = (formData: AdminReceiptFormData, confidence: Record<string, number>) => {
...
export function AdminSmartReceiptFlow() {
  ...
  useEffect(() => {
    track('admin_receipt_flow_started');
  ...
  // Admin-specific form submission
  const onSubmit = useCallback(async (data: AdminReceiptFormData) => {
    trackFormInteraction('admin_form_submit', { 
    ...
    const validator = new AllocationWorkflowValidator(data.amount, workOrders.data || []);
    ...
    if (!validationResult.canSubmit) {
      ...
      trackError(new Error('Validation failed'), { 
        context: 'admin_form_submit_validation',
    ...
    try {
      ...
      track('admin_receipt_submitted', {
        submissionTime,
        hasEmployeeId: !!data.employee_user_id,
        hasNotes: !!data.notes,
        amount: data.amount,
        vendor: data.vendor_name,
        hasOCRData: !!ocrData
      ...
      toast({
        title: 'Receipt Saved!',
        description: 'The admin receipt has been processed and saved.',
      });

    } catch (error: any) {
      console.error('Admin receipt submission error:', error);
      trackError(error, { context: 'Admin receipt submission', formData: data });
      onError();
      toast({
        title: 'Error',
  ...
  }, [workflowAllocations, receiptFile, createAdminReceipt, onSubmitSuccess, actions, form, toast, onError, track, trackError, trackFormInteraction, ocrData, ocrConfidence, workOrders.data]);
  ...
  return (
    <TooltipProvider>
      ...
      {/* Employee Selection Card - Only show for admin context */}
      <Card>
        <CardHeader>
      ...
      {/* Allocation Section - Work order allocation (using all work orders for admin) */}
      <ReceiptAllocationSection
        availableWorkOrders={workOrders.data || []}
        allocations={workflowAllocations}
        totalAmount={currentAmount}
        vendorName={watchedValues.vendor_name}
        onAllocationsChange={handleAllocationsChange}
        showAllocationSection={computed.isFormVisible && workOrders.data && workOrders.data.length > 0}
      />
